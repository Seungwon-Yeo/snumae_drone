...
class fcuModes(Controller):

    def __init__(self):
        pass
    
    def check_takeoff_gps(self):
        print(self.global_pos.x)
        print(self.global_pos.y)
        print("takeoff_gps call Succeeded")

    def setTakeoff(self):
    	rospy.wait_for_service('mavros/cmd/takeoff')
    	try:
    	    takeoffService = rospy.ServiceProxy('mavros/cmd/takeoff', mavros_msgs.srv.CommandTOL)
    	    takeoffService(latitude = self.global_pos.x, longitude = self.global_pos.y, altitude = 2)
