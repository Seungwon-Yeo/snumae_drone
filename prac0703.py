...
def cmd_waypoint():
    ...
    wp = mavros_msgs.msg.Waypoint()
    wp.frame = 0   ## frame = global
    wp.command = 22  ## 22 = takeoff
    wp.is_current = False
    ...
    wp.param4 = 0
    wp.x_lat = 37.453920
    wp.y_long = 128.951774
    wp.z_alt = 0
    wps.append(wp)

    wp = mavros_msgs.msg.Waypoint()
    wp.frame = 0   ## frame = global
    wp.command = 21  ## 21 = land
    wp.is_current = False
    wp.autocontinue = True
    wp.param1 =0
    wp.param2 = 0
    ...
    resp = load_config_srv_prox(start_index = 0, waypoints = wps)

def cmd_waypoint_clear():
    service_name = '/mavros/mission/clear'
    rospy.wait_for_service(service_name)
    load_config_srv_prox = rospy.ServiceProxy(service_name, WaypointClear)
    req_msg = mavros_msgs.srv.WaypointClearRequest()
    resp = load_config_srv_prox(req_msg)
