...
class fcuModes:
    ...
    def setTakeoff(self, global_pos):
        ...
        try:
            ...
            takeoffService(latitude=global_pos.x, longitude=global_pos.y, altitude=4.0)
...
def main():
    ...
    rospy.sleep(0.2)
    sp_glob_pub = rospy.Publisher('mavros/setpoint_raw/global', GlobalPositionTarget, queue_size=10)
    rospy.sleep(0.2)
    ...
    last = rospy.get_time()
    while not rospy.is_shutdown() and (last - start) < 6.0:     
        cnt.updateSp_glob()
        sp_glob_pub.publish(cnt.sp_glob)
